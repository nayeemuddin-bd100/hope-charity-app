...
export default function DonationsPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-6">Manage Donation</h1>
          ...
          {isLoading ? (
            ...
            data?.donations?.map((donation: IDonation) => (
              <TableRow key={donation?._id} className="text-base">
                <TableCell>
                  {donation?.donor?.name?.firstName} {donation?.name?.lastName}
                </TableCell>
                ...
                <TableCell className=" flex flex-col sm:flex-row  gap-3 ">
                  <DeleteConfirmationDialog
                    onConfirm={() => handleDeleteDonation(donation?._id)}
                    itemType="donation"
                  />
                </TableCell>
              </TableRow>
